...
exports.stakingUtils = {
    getEpochFromRaw: (rawEpoch) => {
        const { epoch_id, starting_transaction_hash, starting_block_number, starting_block_timestamp, ending_transaction_hash, ending_block_number, ending_block_timestamp, zrx_deposited, zrx_staked, } = rawEpoch;
        let epochEnd;
    ...
    getEpochWithFeesFromRaw: (rawEpochWithFees) => {
        const { epoch_id, starting_transaction_hash, starting_block_number, starting_block_timestamp, ending_transaction_hash, ending_block_number, ending_block_timestamp, zrx_deposited, zrx_staked, protocol_fees_generated_in_eth, } = rawEpochWithFees;
        let epochEnd;
        if (ending_transaction_hash && ending_block_number) {
            epochEnd = {
                blockNumber: parseInt(ending_block_number, 10),
    ...
    getPoolFromRaw: (rawPool) => {
        const { pool_id, operator, created_at_block_number, created_at_transaction_hash, isVerified, logo_url, location, bio, website, name, } = rawPool;
        return {
            poolId: pool_id,
    ...
    getPoolsFromRaw: (rawPools) => {
        return rawPools.map(exports.stakingUtils.getPoolFromRaw);
    },
    getEpochPoolStatsFromRaw: (rawEpochPoolStats) => {
        const { pool_id, maker_addresses, operator_share, zrx_staked, operator_zrx_staked, member_zrx_staked, share_of_stake, total_protocol_fees_generated_in_eth, share_of_fees, number_of_fills, share_of_fills, approximate_stake_ratio, } = rawEpochPoolStats;
        return {
            poolId: pool_id,
            zrxStaked: Number(zrx_staked || 0),
    ...
    getEpochPoolsStatsFromRaw: (rawEpochPoolsStats) => {
        return rawEpochPoolsStats.map(exports.stakingUtils.getEpochPoolStatsFromRaw);
    },
    getPoolEpochRewardsFromRaw: (rawPoolEpochRewards) => {
    ...
    getPoolProtocolFeesGeneratedFromRaw: (rawPoolProtocolFeesGenerated) => {
    ...
    getPoolsProtocolFeesGeneratedFromRaw: (rawPoolsProtocolFeesGenerated) => {
        return rawPoolsProtocolFeesGenerated.map(exports.stakingUtils.getPoolProtocolFeesGeneratedFromRaw);
    },
    getPoolAvgRewardsFromRaw: (rawPoolAvgRewards) => {
        const { pool_id, avg_member_reward_in_eth, avg_total_reward_in_eth, avg_member_reward_eth_per_zrx, } = rawPoolAvgRewards;
        return {
            poolId: pool_id,
    ...
    getPoolsAvgRewardsFromRaw: (rawPoolsAvgRewards) => {
        return rawPoolsAvgRewards.map(exports.stakingUtils.getPoolAvgRewardsFromRaw);
    },
    getZrxStakedFromRawDelegatorDeposited: (rawDelegatorDeposited) => {
        const resultRow = _.head(rawDelegatorDeposited);
        return resultRow ? parseFloat(resultRow.zrx_deposited) : 0;
    },
    getDelegatorStakedFromRaw: (rawDelegatorStaked) => {
        const firstRow = _.head(rawDelegatorStaked);
        const zrxStaked = (firstRow && parseFloat(firstRow.zrx_staked_overall)) || 0;
        const poolData = rawDelegatorStaked.map(row => ({
            poolId: row.pool_id,
    ...
    getDelegatorAllTimeStatsFromRaw: (rawDelegatorAllTimeStats) => {
        const poolData = rawDelegatorAllTimeStats.map(rawStats => ({
            poolId: rawStats.pool_id,
            rewardsInEth: parseFloat(rawStats.reward) || 0,
        }));
        return poolData;
    },
    getDelegatorEventsFromRaw: (rawDelegatorEvents) => {
        const delegatorEvents = rawDelegatorEvents.map(rawEvent => ({
    ...
    getAlltimePoolRewards: (rawAllTimePoolRewards, rawPoolsProtocolFeesGenerated) => {
        return {
    ...
    getAllTimeStakingStatsFromRaw: (rawAllTimeAllTimeStats) => {
...
//# sourceMappingURL=staking_utils.js.map