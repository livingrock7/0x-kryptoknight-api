...
const ethers = require("ethers"); // HACK to make ethers quiet
...
describe(SUITE_NAME, () => {
    describe('.getSortedSignersByAvailability', () => {
        it('sorts signers in order of highest balance andd lowest tx count', () => {
            ...
            const calculated = transaction_watcher_signer_service_1.TransactionWatcherSignerService.getSortedSignersByAvailability(testCase);
            contracts_test_utils_1.expect(calculated).to.be.deep.equal(expected);
        });
    });
});
//# sourceMappingURL=transaction_watcher_signer_test.js.map